...
class SimpleSearchEngine:
    ...
    def evaluate(self, query: str, options: Dict[str, Any], ranker: Ranker) -> Iterator[Dict[str, Any]]:
        ...
        # Count the number of times each term appears in the query
        multiplicites = Counter(terms)

        # Use only unique terms for retrieval
        terms = list(multiplicites.keys())

        t: float = options.get("match_threshold", 1.0)  # defaults to only accepting perfect matches
        m: int = len(terms)
        n: int = max(1, min(m, int(t * m)))

        max_documents: int = options.get("hit_count", 10) # defaults to 10 (idk if this is correct)
            ...
            if counter[doc_id] >= n:
                ...
                for term, posting in zip(terms, postings):
                    if posting and posting.document_id == doc_id:
                        ranker.update(term, multiplicites.get(term), posting)
